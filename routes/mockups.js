...
router.get('/', (req, res) => {
  const db   = getDB();
  const rows = db.prepare('SELECT * FROM mockups ORDER BY updated_at DESC').all();
  res.json(rows.map(r => ({ ...r, views: JSON.parse(r.views_json || '[]') })));
});

// GET /api/mockups/product/:product — first mockup for a given product
router.get('/product/:product', (req, res) => {
  const db  = getDB();
  const row = db.prepare('SELECT * FROM mockups WHERE product=? ORDER BY updated_at DESC LIMIT 1').get(req.params.product);
...
router.post('/', requireAuth, (req, res) => {
  ...
  const info = db.prepare(`
    INSERT INTO mockups (name, product, views_json, file3d_name, file3d_url)
    VALUES (?,?,?,?,?)
...
// PUT /api/mockups/:id (admin)
router.put('/:id', requireAuth, (req, res) => {
  const db  = getDB();
  const row = db.prepare('SELECT id FROM mockups WHERE id=?').get(req.params.id);
  if (!row) return res.status(404).json({ error: 'Not found' });
  ...
  db.prepare(`
    ...
    WHERE id=?
  `).run(name, product, JSON.stringify(views || []), file3d_name || '', file3d_url || '', req.params.id);
...
// DELETE /api/mockups/:id (admin)
router.delete('/:id', requireAuth, (req, res) => {
  const db = getDB();
  db.prepare('DELETE FROM mockups WHERE id=?').run(req.params.id);
...
router.post('/:id/upload-glb', requireAuth, glbUpload.single('file'), (req, res) => {
  ...
  db.prepare("UPDATE mockups SET file3d_url=?, file3d_name=?, updated_at=datetime('now') WHERE id=?")
    .run(url, req.file.originalname, req.params.id);
